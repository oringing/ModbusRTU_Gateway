...
/* USER CODE BEGIN UART_Printf */
int fputc(int ch, FILE *f)
{
  // 使用较短超时时间，避免长时间阻塞
  if(HAL_UART_Transmit(&huart1, (uint8_t *)&ch, 1, 2) != HAL_OK)
  {
    // 如果发送失败，直接返回，不阻塞等待
    return -1;
  }
...
int main(void)
{
  ...
  osThreadDef(LED_Task, Start_LED_Task, osPriorityLow, 0, 128);
  LED_TaskHandle = osThreadCreate(osThread(LED_Task), NULL);

  /* definition and creation of UART_Task */
  osThreadDef(UART_Task, Start_UART_Task, osPriorityNormal, 0, 256);
...
void Start_LED_Task(void const * argument)
{
  ...
  for(;;)
  {
    ...
    osDelay(500); // 延时500ms，FreeRTOS的延时函数
  }
  /* USER CODE END 5 */
}
...
void Start_UART_Task(void const * argument)
{
  ...
  for(;;)
  {
    // 这里写你的电机控制逻辑，比如读取电位器ADC值，控制电机转动
    // 先简单写个串口打印，测试任务是否在运行
    // 先用非阻塞方式测试
    char msg[] = "Motor Task Running\r\n";
    HAL_UART_Transmit(&huart1, (uint8_t*)msg, sizeof(msg)-1, 10);
