...
// 通过UART输出错误/关键日志消息（始终启用，不受宏控制）
static void System_Error_Log(const char* msg) {
    if (msg == NULL) {
        return;
    }
...
// 通过UART输出监控日志消息（受SYSTEM_STACK_WATERMARK_LOG_ENABLE宏控制）
...
static void System_IWDG_Init(void) {
    ...
    if (HAL_IWDG_Init(&hiwdg) != HAL_OK) {
        ...
        System_Error_Log("ERR: IWDG init failed\r\n");
    }
#endif
}
...
void System_Check_Reset_Source(void) {
    ...
    // 2. 首次上电无复位标志
    if (!reset_detected) {
        System_Error_Log("[RESET] First power-on (no reset flags)\r\n");
    }
